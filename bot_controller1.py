...
class EnhancedBotController:
    ...
    def validate_account(self, account: Dict) -> Tuple[bool, Optional[float], Optional[str]]:
        ...
        try:
            ...
            # FIXED: Handle both dict and response object properly
            if hasattr(resp, 'json'):
                # If resp has .json() method, call it
                resp = resp.json()
                ...
                if ret_code == 0:
                    ...
                    if isinstance(result, dict):
                        ...
                        if balance_list and isinstance(balance_list, list) and len(balance_list) > 0:
                            total_equity = float(balance_list[0].get("totalEquity", 0))
                            self.logger.info(f"✅ Account validated successfully - Balance: ${total_equity:.2f}")
                            return True, total_equity, None
                        else:
                            self.logger.warning(f"⚠️ Account validated but no balance list found in result")
                            return True, 0.0, "No balance data"
                    else:
                        self.logger.warning(f"⚠️ Account validated but result is not dict: {type(result)}")
                        return True, 0.0, "Result not dict"
                else:
                    error_msg = resp.get("retMsg", "Unknown error")
                    self.logger.error(f"❌ Account validation failed: {error_msg} (Code: {ret_code})")
                    return False, None, f"{error_msg} (Code: {ret_code})"
            else:
