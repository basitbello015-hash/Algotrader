...
async def status():
    ...
    response = {
        ...
        "accounts": {
            ...
            "configuration_method": account_stats.get("configuration_method", "unknown")
        },
        "market_insights": {
    ...
    logger.info(
        f"📊 Status: Bot running={bc.is_running()}, "
        f"Trades today={bc.trades_today}, "
        f"Accounts={account_stats.get('total_accounts', 0)}"
    )
    return response
...
async def update_ai_coins():
    ...
    try:
        ...
        coins = bc.ai_selector.update_from_groq()
        elapsed = time.time() - start_time
        
        logger.info(f"✅ AI coins updated in {elapsed:.2f}s: {len(coins)} symbols")
...
# New API endpoints for environment variable accounts
...
@app.post("/api/accounts/validate-all")
async def validate_all_accounts_endpoint():
    """Validate all accounts"""
    from services.accounts_service import validate_all_accounts
    try:
...
@app.get("/api/accounts/env-status")
async def env_status_endpoint():
    """Get environment variables status"""
    from services.accounts_service import get_environment_status
    try:
...
@app.post("/api/accounts/sync-bot")
async def sync_bot_endpoint():
    """Sync accounts with bot controller"""
    from services.accounts_service import sync_accounts_with_bot
    try:
...
@app.post("/api/accounts/demo")
async def demo_account_endpoint():
    """Add demo account for testing"""
    from services.accounts_service import add_demo_account
    try:
...
async def startup():
    ...
    from services.accounts_service import load_accounts_from_env, get_environment_status
    ...
    logger.info(f"   Groq API configured: {env_status.get('groq', {}).get('has_key', False)}")
    
    if not accounts:
        logger.warning("⚠️ No accounts found in environment variables!")
        logger.info("ℹ️ To add accounts, set these environment variables:")
        logger.info("   BYBIT_API_KEY=your_api_key_here")
        logger.info("   BYBIT_API_SECRET=your_api_secret_here")
        logger.info("   Or for multiple accounts: BYBIT_API_KEY_1, BYBIT_API_SECRET_1, etc.")
    else:
        logger.info("✅ Accounts loaded from environment variables")
