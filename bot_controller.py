    ...
    def update_from_groq(self) -> List[str]:
        ...
        try:
            ...
            if coins and len(coins) >= 5:
                ...
                return self.save_coins(fallback_coins, market_analysis)
            
        except Exception as e:
            self.logger.error(f"❌ Groq update failed: {e}", exc_info=True)
            elapsed = time.time() - start_time
            self.logger.warning(f"⏱️ Failed after {elapsed:.2f}s, using defaults")
            
            return self.save_coins(DEFAULT_AI_COINS, {
...
class EnhancedBotController:
    ...
    def _start_scheduler(self):
        ...
        def daily_update():
            ...
            confidence = insights.get("market_analysis", {}).get("confidence_score", 0)
            
            self.logger.info(f"✅ Groq update completed in {elapsed:.2f}s")
            self.logger.info(f"📊 Market sentiment: {sentiment} (confidence: {confidence:.0%})")
        ...
        with self._file_lock:
            try:
                with open(ACCOUNTS_FILE, "r") as f:
                    ...
                    elapsed = time.time() - start_time
                    self.logger.debug(f"📖 Loaded {len(accounts)} accounts in {elapsed:.3f}s")
                    return accounts
            except Exception as e:
    ...
    def save_accounts(self, accounts: List[Dict]):
        ...
        with self._file_lock:
            try:
                ...
                elapsed = time.time() - start_time
                self.logger.debug(f"💾 Saved {len(accounts)} accounts in {elapsed:.3f}s")
            except Exception as e:
    ...
    def _get_client(self, account: Dict) -> Optional[HTTP]:
        ...
        try:
            key = account.get("api_key") or account.get("apiKey")
            secret = account.get("api_secret") or account.get("apiSecret")
    ...
    def get_coins(self) -> List[str]:
        ...
        self.logger.debug(f"📊 Active Groq coins: {len(coins)} symbols")
        return coins
    ...
    def get_market_insights(self) -> Dict[str, Any]:
        ...
        # Log insights
        sentiment = insights.get("market_analysis", {}).get("market_sentiment", "unknown")
        risk = insights.get("market_analysis", {}).get("risk_level", "medium")
        
        self.logger.info(f"📊 Groq Insights: {sentiment.upper()} sentiment, {risk.upper()} risk")
        
        return insights
    
    def force_ai_update(self) -> Dict[str, Any]:
        """Force immediate AI update from Groq"""
        self.logger.info("🔄 Manual Groq update requested")
        ...
        try:
            ...
            self.logger.info(f"✅ Manual Groq update completed in {elapsed:.2f}s")
            ...
            return {
                ...
                "message": f"Groq update successful - {len(coins)} coins selected"
            }
    ...
    def scan_symbol(self, client: HTTP, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not rsi or rsi > 40:
                self.logger.debug(f"📊 {symbol}: RSI too high ({rsi:.1f})")
                return None
            ...
            # Calculate score
            base_score = 7.0
            volume_score = min(volume_ratio - 1.0, 2.0)
            rsi_score = max(0, (40 - rsi) / 10)
            score = base_score + volume_score + rsi_score
            ...
            self.logger.debug(
                ...
                f"Price: ${current_price:.4f}, Scan: {scan_time:.3f}s"
            )
    ...
    def find_opportunity(self, client: HTTP) -> Optional[Dict]:
        ...
        for symbol, future in futures:
            try:
                ...
                scan_results.append({
                    ...
                    "qualified": result is not None and result.get("score", 0) >= 7.0
                })
        ...
        self.logger.info(
            f"📊 Scan completed: {total_scans} symbols, "
            f"{successful_scans} successful, "
            f"{qualified} qualified in {scan_time:.2f}s "
            f"(avg: {avg_scan_time:.2f}s)"
        )
        
        if qualified > 0:
            self.logger.info(f"🎯 Found {qualified} qualified opportunities")
        ...
        self.logger.debug("📭 No qualified opportunities found")
        return None
    ...
    def execute_trade(self, client: HTTP, account: Dict, opportunity: Dict) -> bool:
        ...
        self.logger.info("=" * 50)
        self.logger.info(f"🚀 EXECUTING TRADE: {symbol}")
        self.logger.info("=" * 50)
        ...
        # LIVE TRADE
            ...
            try:
                ...
                if order_resp.get("retCode") == 0:
                    ...
                    error_msg = order_resp.get("retMsg", "Unknown error")
    ...
    def _save_trade(self, trade: Dict):
        ...
        try:
            ...
            with self._file_lock:
                ...
                self.logger.info(f"📝 Trade {trade.get('id')} added to history")
    ...
    def start(self):
        ...
        for account in valid_accounts:
            self.logger.info(f"   ✅ {account.get('name')}: ${account.get('balance', 0):.2f}")
    ...
    def stop(self):
        ...
        self.logger.info(f"   🎯 Qualified Signals: {self.stats['qualified_signals']}")
        ...
        while self._running and not self._stop.is_set():
            ...
            try:
                ...
                for account in valid_accounts:
                    ...
                    if opportunity:
                        self.logger.info(f"🎯 Opportunity found for {account_name}: {opportunity['symbol']}")
